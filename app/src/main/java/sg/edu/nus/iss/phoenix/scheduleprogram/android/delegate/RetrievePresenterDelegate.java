    ...
    protected void onPostExecute(String result) {
        ...
        if (result != null && !result.equals("")) {
            try {
                ...
                JSONArray presenterArray = reader.getJSONArray("presenterList");

                for (int i = 0; i < presenterArray.length(); i++) {
                    JSONObject presenterJson = presenterArray.getJSONObject(i);
                    String id = presenterJson.getString("id");
                    String name = presenterJson.getString("name");

                    presenters.add(new Presenter(id,name));
                }
        ...
        if (reviewSelectPresenterProducerController != null)
            reviewSelectPresenterProducerController.presentersRetrieved(presenters);
    }
